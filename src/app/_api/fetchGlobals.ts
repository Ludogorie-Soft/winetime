...
import { FOOTER_QUERY, HEADER_QUERY, HOMESETTINGS_QUERY, SETTINGS_QUERY } from '../_graphql/globals'
...
export async function fetchSettings(): Promise<Settings> {
  ...
  const settings = await fetch(`${GRAPHQL_API_URL}/api/graphql`, {
    ...
    ?.then(res => {
      if (!res.ok) throw new Error(`Error fetching doc: ${res.status} ${res.statusText}`);
      return res.json()
    })
    ?.then(res => {
...
export async function fetchHomepageSettings(): Promise<Settings> {
  if (!process.env.NEXT_PUBLIC_SERVER_URL) throw new Error('NEXT_PUBLIC_SERVER_URL not found')
