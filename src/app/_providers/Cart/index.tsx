...
export type CartContext = {
  ...
  setCalculatedData: (value: number) => void
...
export const CartProvider = props => {
  ...
  useEffect(() => {
    ...
    const newTotal =
      cart?.items?.reduce((acc, item) => {
        // Check if item.product is an object and item.product.price is a number
        const price =
          typeof item.product === 'object'
            ? typeof item.product.discountPrice === 'number'
              ...
              : item.product.price
            : 0
        const itemTotal = price * (typeof item.quantity === 'number' ? item.quantity : 0)
        return acc + itemTotal
      }, 0) || 0

    setTotal({
      formatted: newTotal.toLocaleString('bg-BG', {
        style: 'currency',
        currency: 'BGN',
      }),
      raw: newTotal,
  ...
  useEffect(() => {
    ...
    const newTotalWithCalculatedData = total.raw + calculatedData

    setTotalPrice({
      formatted: newTotalWithCalculatedData.toLocaleString('bg-BG', {
        style: 'currency',
        currency: 'BGN',
      }),
      raw: newTotalWithCalculatedData,
    })
  }, [total, calculatedData, hasInitialized])
