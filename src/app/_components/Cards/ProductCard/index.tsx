...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  return (
    <div className="w-full relative h-96 shadow-lg shadow-gray-200 border-2 rounded border-gray-200">
      <div className="absolute z-10 right-0 top-[3.3%] mid:top-[4%]">
        <AddToFavouritesButton product={product} />
      </div>
      <Link
        href={`/products/${product.slug}`}
        className="relative h-96 w-full flex flex-col items-center justify-center gap-4 cursor-pointer"
      >
        <div className="relative w-full min-h-72 flex items-center justify-center">
          {metaImage ? (
            <>
              <Image
                src={`/media/${metaImage}`}
              ...
              {product.isChosen && (
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger>
                      <div className="absolute bottom-[80px] left-[16px]">
                        <Image src="/wine-icon.svg" alt="Homemade" width={90} height={90} />
                      </div>
                    </TooltipTrigger>
                    <TooltipContent>
        ...
        <div className="w-full py-2 flex flex-col gap-1 items-start p-2">
